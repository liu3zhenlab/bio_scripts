...
use constant INDEL_PENALTY => 2;				# Default penalty of an INDEL
...
while (<IN>) {
	...
	if (!/^@/) { # not header
		...
		## criterion: mapped locations:
			if ($_ =~ /NH\:i\:(\d+)/) {
				my $mapped_loci = $1;
				if ($mapped_loci <= $maxloci) {
					$maxloci_pass = 1;
				}
			} else {
				print STDERR "WARNING: $line[0] DONOT have NH (number of reported alignments) information.\n";
			}
		## criterion: mismatch
